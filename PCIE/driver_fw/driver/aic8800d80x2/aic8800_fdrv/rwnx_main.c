...
int handle_private_cmd(struct net_device *net, char *command, u32 cmd_len)
{
    ...
    do {
        ...
        #endif
		#ifdef AICWF_PCIE_SUPPORT
        struct rwnx_hw *p_rwnx_hw = g_rwnx_plat->pcidev->rwnx_hw;
        #endif
        #ifdef CONFIG_RFTEST
        if (strcasecmp(argv[0], "GET_RX_RESULT") ==0) {
		...
		buf[1] = (u8_l)freq;
        ...
        } else if (strcasecmp(argv[0], "SET_RXMETER") == 0) {
            printk("set_rx_meter\n");
            freq = (int)command_strtoul(argv[1], NULL, 10);
            rwnx_send_rftest_req(p_rwnx_hw, SET_RXMETER, sizeof(freq), (u8_l *)&freq, NULL);
        } else if (strcasecmp(argv[0], "SET_FREQ_CAL") == 0) {
            printk("set_freq_cal\n");
            if (argc < 2) {
                printk("wrong param\n");
            ...
            getefuse_param.block = command_strtoul(argv[1], NULL, 10);
            rwnx_send_rftest_req(p_rwnx_hw, GET_EFUSE, sizeof(cmd_rf_getefuse_t), (u8_l *)&getefuse_param, &cfm);
            ...
            memcpy(command, &cfm.rftest_result[0], 4);
            bytes_written = 4;
            printk("cap=0x%x, cap_fine=0x%x\n", (unsigned int)(cfm.rftest_result[0] & 0x0000ffff), (unsigned int)((cfm.rftest_result[0] >> 16) & 0x0000ffff));
        } else if (strcasecmp(argv[0], "RDWR_PWRMM") == 0) {
	...
	bytes_written = 6 * 12;
			...
			if(p_rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80)
				res_len = 3 * 3 + 3 * 6;
			else if(p_rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80X2)
				res_len = (3 * 3 + 3 * 6) * 2;
			else{
				res_len = 3 + 4;
			}
		...
		}else if (strcasecmp(argv[0], "SET_ANT") == 0) {//"<val> = 0/ant0, 1/ant1, 2/both"
			u8_l func = 0;
			if (argc > 1) {
				func = command_strtoul(argv[1], NULL, 10);
				AICWFDBG(LOGINFO, "ant %d\r\n", func);
				rwnx_send_rftest_req(g_rwnx_plat->pcidev->rwnx_hw, SET_ANT_MODE, sizeof(func), (u8_l *)&func, NULL);
			} else {
				return -EINVAL;
			}
			return 0;
