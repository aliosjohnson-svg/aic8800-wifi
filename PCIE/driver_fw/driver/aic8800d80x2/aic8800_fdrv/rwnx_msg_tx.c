						 ...
						 int reqcfm, lmac_msg_id_t reqid, void *cfm)
{
	...
	if((rwnx_hw->pcidev->bus_if->state == BUS_DOWN_ST) || (rwnx_hw->pci_suspending && reqid != MM_SET_RESUME_RESTORE_CFM)) {
		rwnx_msg_free(rwnx_hw, msg_params);
		pcie_err("bus is down\n");
		return 0;
	}
#endif
...
#endif
#ifdef AICWF_PCIE_SUPPORT
		aicwf_set_cmd_tx((void *)(rwnx_hw->pcidev), cmd->a2e_msg, sizeof(struct lmac_msg) + cmd->a2e_msg->param_len);
#endif
	}
...
int rwnx_send_reset(struct rwnx_hw *rwnx_hw)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, MM_RESET_REQ, NULL, 0);
...
int rwnx_send_start(struct rwnx_hw *rwnx_hw)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, MM_START_REQ, start_req_param, sizeof(struct mm_start_req));
...
int rwnx_send_coex_req(struct rwnx_hw *rwnx_hw, u8_l disable_coexnull, u8_l enable_nullcts)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_COEX_REQ, coex_req, sizeof(struct mm_set_coex_req));
					...
					u8_l fwtrace_redir_en, struct mm_set_stack_start_cfm *cfm)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_STACK_START_REQ, req, sizeof(struct mm_set_stack_start_req));
...
int rwnx_send_vendor_swconfig_req(struct rwnx_hw *rwnx_hw, uint32_t swconfig_id, int32_t *param_in, int32_t *param_out)
{
    ...
    switch (swconfig_id)
    {
        case BCN_CFG_REQ:
            ...
            rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_VENDOR_SWCONFIG_REQ, req, sizeof(struct mm_set_vendor_swconfig_req));
...
int rwnx_send_txpwr_lvl_v3_req(struct rwnx_hw *rwnx_hw)
{
    ...
    if (txpwr_lvl_v3->enable == 0) {
        ...
        rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_TXPWR_IDX_LVL_REQ, txpwr_lvl_req, sizeof(struct mm_set_txpwr_lvl_req));
...
int rwnx_send_txpwr_lvl_v4_req(struct rwnx_hw *rwnx_hw)
{
    ...
    if (txpwr_loss->loss_enable == 1) {
        ...
        for (i = 0; i <= 11; i++)
            txpwr_lvl_v4->pwrlvl_11b_11ag_2g4[i] += txpwr_loss->loss_value;
        for (i = 0; i <= 9; i++)
            txpwr_lvl_v4->pwrlvl_11n_11ac_2g4[i] += txpwr_loss->loss_value;
        for (i = 0; i <= 11; i++)
            txpwr_lvl_v4->pwrlvl_11ax_2g4[i] += txpwr_loss->loss_value;

		for (i = 0; i <= 7; i++)
            txpwr_lvl_v4->pwrlvl_11a_5g[i] += txpwr_loss->loss_value;
        for (i = 0; i <= 9; i++)
            txpwr_lvl_v4->pwrlvl_11n_11ac_5g[i] += txpwr_loss->loss_value;
        for (i = 0; i <= 11; i++)
            txpwr_lvl_v4->pwrlvl_11ax_5g[i] += txpwr_loss->loss_value;
    }
...
int rwnx_send_txpwr_lvl_adj_req(struct rwnx_hw *rwnx_hw)
{
    ...
    if (txpwr_lvl_adj->enable == 0) {
        ...
        rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_TXPWR_LVL_ADJ_REQ, txpwr_lvl_adj_req, sizeof(struct mm_set_txpwr_lvl_adj_req));
...
int rwnx_send_txpwr_ofst2x_req(struct rwnx_hw *rwnx_hw)
{
    ...
    if (txpwr_ofst2x->enable){
        ...
        rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_TXPWR_OFST_REQ, txpwr_ofst_req, sizeof(struct mm_set_txpwr_ofst_req));
...
int rwnx_send_me_config_req(struct rwnx_hw *rwnx_hw)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, ME_CONFIG_REQ, req, sizeof(struct me_config_req));
...
int rwnx_send_me_chan_config_req(struct rwnx_hw *rwnx_hw)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, ME_CHAN_CONFIG_REQ, req, sizeof(struct me_chan_config_req));
...
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0) && LINUX_VERSION_CODE < KERNEL_VERSION(4, 20, 0))
	if (params->he_capa) {
		const struct ieee80211_he_cap_elem_4_19 *he_capa = (const struct ieee80211_he_cap_elem_4_19 *) params->he_capa;
		struct ieee80211_he_mcs_nss_supp *mcs_nss_supp =
								(struct ieee80211_he_mcs_nss_supp *)(he_capa + 1);
...
{
	struct mm_all_restore_param *all_param_req;
//	uint8_t *ht_mcs;
//	int i;
	int error;
//	struct wiphy *wiphy = rwnx_hw->wiphy;
...
//	struct ieee80211_sta_he_cap const *he_cap = NULL;
	...
	/* Build the MM_SET_RESUME_RESTORE_REQ message */
	all_param_req = rwnx_msg_zalloc(MM_SET_RESUME_RESTORE_REQ, TASK_MM, DRV_TASK_ID, sizeof(struct mm_all_restore_param));
...
#if 0
	all_param_req->stack_req.is_stack_start = on;
	all_param_req->stack_req.efuse_valid = efuse_valid;
	all_param_req->stack_req.set_vendor_info = set_vendor_info;
	all_param_req->stack_req.fwtrace_redir = fwtrace_redir_en;
	...
	//********************config
	all_param_req->config_req.ht_supp = ht_cap->ht_supported;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0) || defined(CONFIG_VHT_FOR_OLD_KERNEL)
	all_param_req->config_req.vht_supp = vht_cap->vht_supported;
#endif
	all_param_req->config_req.ht_cap.ht_capa_info = cpu_to_le16(ht_cap->cap | IEEE80211_HT_CAP_LDPC_CODING);
	all_param_req->config_req.ht_cap.a_mpdu_param = ht_cap->ampdu_factor |
									 (ht_cap->ampdu_density <<
										 IEEE80211_HT_AMPDU_PARM_DENSITY_SHIFT);
	for (i = 0; i < sizeof(ht_cap->mcs); i++)
		all_param_req->config_req.ht_cap.mcs_rate[i] = ht_mcs[i];
	all_param_req->config_req.ht_cap.ht_extended_capa = 0;
	all_param_req->config_req.ht_cap.tx_beamforming_capa = 0;
	all_param_req->config_req.ht_cap.asel_capa = 0;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0) || defined(CONFIG_VHT_FOR_OLD_KERNEL)
	if (all_param_req->config_req.vht_supp) {
		all_param_req->config_req.vht_cap.vht_capa_info = cpu_to_le32(vht_cap->cap);
		all_param_req->config_req.vht_cap.rx_highest = cpu_to_le16(vht_cap->vht_mcs.rx_highest);
		all_param_req->config_req.vht_cap.rx_mcs_map = cpu_to_le16(vht_cap->vht_mcs.rx_mcs_map);
		all_param_req->config_req.vht_cap.tx_highest = cpu_to_le16(vht_cap->vht_mcs.tx_highest);
		all_param_req->config_req.vht_cap.tx_mcs_map = cpu_to_le16(vht_cap->vht_mcs.tx_mcs_map);
	}
#endif
	...
	he_cap = &rwnx_he_capa.he_cap;
	#endif
	all_param_req->config_req.he_supp = he_cap->has_he;
	for (i = 0; i < ARRAY_SIZE(he_cap->he_cap_elem.mac_cap_info); i++) {
		all_param_req->config_req.he_cap.mac_cap_info[i] = he_cap->he_cap_elem.mac_cap_info[i];
	}
	for (i = 0; i < ARRAY_SIZE(he_cap->he_cap_elem.phy_cap_info); i++) {
		all_param_req->config_req.he_cap.phy_cap_info[i] = he_cap->he_cap_elem.phy_cap_info[i];
	}
	all_param_req->config_req.he_cap.mcs_supp.rx_mcs_80 = cpu_to_le16(he_cap->he_mcs_nss_supp.rx_mcs_80);
	all_param_req->config_req.he_cap.mcs_supp.tx_mcs_80 = cpu_to_le16(he_cap->he_mcs_nss_supp.tx_mcs_80);
	all_param_req->config_req.he_cap.mcs_supp.rx_mcs_160 = cpu_to_le16(he_cap->he_mcs_nss_supp.rx_mcs_160);
	all_param_req->config_req.he_cap.mcs_supp.tx_mcs_160 = cpu_to_le16(he_cap->he_mcs_nss_supp.tx_mcs_160);
	all_param_req->config_req.he_cap.mcs_supp.rx_mcs_80p80 = cpu_to_le16(he_cap->he_mcs_nss_supp.rx_mcs_80p80);
	all_param_req->config_req.he_cap.mcs_supp.tx_mcs_80p80 = cpu_to_le16(he_cap->he_mcs_nss_supp.tx_mcs_80p80);
	for (i = 0; i < MAC_HE_PPE_THRES_MAX_LEN; i++) {
		all_param_req->config_req.he_cap.ppe_thres[i] = he_cap->ppe_thres[i];
	}
	all_param_req->config_req.he_ul_on = rwnx_hw->mod_params->he_ul_on;
	//}
#else
	all_param_req->config_req.he_supp = false;
	all_param_req->config_req.he_ul_on = false;
#endif
	all_param_req->config_req.ps_on = rwnx_hw->mod_params->ps_on;
	all_param_req->config_req.dpsm = rwnx_hw->mod_params->dpsm;
	all_param_req->config_req.tx_lft = rwnx_hw->mod_params->tx_lft;
	//all_param_req->config_req.amsdu_tx = rwnx_hw->mod_params->amsdu_tx;
	all_param_req->config_req.ant_div_on = rwnx_hw->mod_params->ant_div;
	if (rwnx_hw->mod_params->use_80)
		all_param_req->config_req.phy_bw_max = PHY_CHNL_BW_80;
	else if (rwnx_hw->mod_params->use_2040)
		all_param_req->config_req.phy_bw_max = PHY_CHNL_BW_40;
	else
		all_param_req->config_req.phy_bw_max = PHY_CHNL_BW_20;

	wiphy_info(wiphy, "HT supp %d, VHT supp %d, HE supp %d\n", all_param_req->config_req.ht_supp,
					all_param_req->config_req.vht_supp, all_param_req->config_req.he_supp);
	//}

	//*********************chan_config
	all_param_req->chan_config_req.chan2G4_cnt =  0;
	if (wiphy->bands[NL80211_BAND_2GHZ] != NULL) {
		struct ieee80211_supported_band *b = wiphy->bands[NL80211_BAND_2GHZ];
		for (i = 0; i < b->n_channels; i++) {
			all_param_req->chan_config_req.chan2G4[all_param_req->chan_config_req.chan2G4_cnt].flags = 0;
			if (b->channels[i].flags & IEEE80211_CHAN_DISABLED)
				all_param_req->chan_config_req.chan2G4[all_param_req->chan_config_req.chan2G4_cnt].flags |= CHAN_DISABLED;
			all_param_req->chan_config_req.chan2G4[all_param_req->chan_config_req.chan2G4_cnt].flags |= get_chan_flags(b->channels[i].flags);
			all_param_req->chan_config_req.chan2G4[all_param_req->chan_config_req.chan2G4_cnt].band = NL80211_BAND_2GHZ;
			all_param_req->chan_config_req.chan2G4[all_param_req->chan_config_req.chan2G4_cnt].freq = b->channels[i].center_freq;
			all_param_req->chan_config_req.chan2G4[all_param_req->chan_config_req.chan2G4_cnt].tx_power = chan_to_fw_pwr(b->channels[i].max_power);
			all_param_req->chan_config_req.chan2G4_cnt++;
			if (all_param_req->chan_config_req.chan2G4_cnt == MAC_DOMAINCHANNEL_24G_MAX)
				break;
		}
	}

	all_param_req->chan_config_req.chan5G_cnt = 0;
	if (wiphy->bands[NL80211_BAND_5GHZ] != NULL) {
		struct ieee80211_supported_band *b = wiphy->bands[NL80211_BAND_5GHZ];
		for (i = 0; i < b->n_channels; i++) {
			all_param_req->chan_config_req.chan5G[all_param_req->chan_config_req.chan5G_cnt].flags = 0;
			if (b->channels[i].flags & IEEE80211_CHAN_DISABLED)
				all_param_req->chan_config_req.chan5G[all_param_req->chan_config_req.chan5G_cnt].flags |= CHAN_DISABLED;
			all_param_req->chan_config_req.chan5G[all_param_req->chan_config_req.chan5G_cnt].flags |= get_chan_flags(b->channels[i].flags);
			all_param_req->chan_config_req.chan5G[all_param_req->chan_config_req.chan5G_cnt].band = NL80211_BAND_5GHZ;
			all_param_req->chan_config_req.chan5G[all_param_req->chan_config_req.chan5G_cnt].freq = b->channels[i].center_freq;
			all_param_req->chan_config_req.chan5G[all_param_req->chan_config_req.chan5G_cnt].tx_power = chan_to_fw_pwr(b->channels[i].max_power);
			all_param_req->chan_config_req.chan5G_cnt++;
			if (all_param_req->chan_config_req.chan5G_cnt == MAC_DOMAINCHANNEL_5G_MAX)
				break;
		}
	}
	...
	//********************start
	memcpy(&all_param_req->start_req.phy_cfg, &rwnx_hw->phy.cfg, sizeof(rwnx_hw->phy.cfg));
	all_param_req->start_req.uapsd_timeout = (u32_l)rwnx_hw->mod_params->uapsd_timeout;
	all_param_req->start_req.lp_clk_accuracy = (u16_l)rwnx_hw->mod_params->lp_clk_ppm;
#else
	memcpy(all_param_req, &rwnx_hw->all_restore_param, sizeof(struct mm_all_restore_param));
#endif

	//*******************add_if
	if(vif != NULL) {
		memcpy(&(all_param_req->add_if_req.addr.array[0]), vif->wdev.address, ETH_ALEN);
		all_param_req->sub_msg_id[all_param_req->sub_msg_cnt] = MM_ADD_IF_REQ;
		all_param_req->sub_msg_cnt++;
		all_param_req->add_vif_cnt = 1;
		switch (RWNX_VIF_TYPE(vif)) {
		#ifdef CONFIG_RWNX_FULLMAC
		//case NL80211_IFTYPE_P2P_DEVICE:
		case NL80211_IFTYPE_P2P_CLIENT:
			all_param_req->add_if_req.p2p = true;
			// no break
		#endif /* CONFIG_RWNX_FULLMAC */
		case NL80211_IFTYPE_STATION:
			all_param_req->add_if_req.type = MM_STA;
			break;
		case NL80211_IFTYPE_ADHOC:
			all_param_req->add_if_req.type = MM_IBSS;
			break;
		#ifdef CONFIG_RWNX_FULLMAC
		case NL80211_IFTYPE_P2P_GO:
			all_param_req->add_if_req.p2p = true;
		...
		#endif /* CONFIG_RWNX_FULLMAC */
		case NL80211_IFTYPE_AP:
			all_param_req->add_if_req.type = MM_AP;
			break;
		case NL80211_IFTYPE_MESH_POINT:
			all_param_req->add_if_req.type = MM_MESH_POINT;
			break;
		case NL80211_IFTYPE_AP_VLAN:
			return -1;
		case NL80211_IFTYPE_MONITOR:
			all_param_req->add_if_req.type = MM_MONITOR;
			break;
		default:
			all_param_req->add_if_req.type = MM_STA;
			break;
	...
	/* Send the MM_SET_RESUME_RESTORE_REQ  message to UMAC FW */
	error = rwnx_send_msg(rwnx_hw, all_param_req, 0, MM_SET_RESUME_RESTORE_CFM, NULL);
...
int rwnx_prepare_resume_restore_req(struct rwnx_hw *rwnx_hw, lmac_msg_id_t const id, void *param, u16_l const param_len)
{
    ...
    if (id == MM_SET_VENDOR_SWCONFIG_REQ) {
        struct mm_set_vendor_swconfig_req *req = (struct mm_set_vendor_swconfig_req *)param;
        u32_l swconfig_id = req->swconfig_id;
        switch (swconfig_id) {
        case BCN_CFG_REQ:
        case TEMP_COMP_SET_REQ:
        case EXT_FLAGS_SET_REQ:
        case EXT_FLAGS_MASK_SET_REQ: {
            for (i = 0; i < rwnx_hw->all_restore_param.vendor_swconfig_req_cnt; i++) {
                if (rwnx_hw->all_restore_param.vendor_swconfig_req[i].swconfig_id == swconfig_id) {
                    found = true;
                    memcpy(&rwnx_hw->all_restore_param.vendor_swconfig_req[i], req, sizeof(struct mm_set_vendor_swconfig_req));
                    break;
                }
            }
            if (found == false) {
                rwnx_hw->all_restore_param.sub_msg_id[rwnx_hw->all_restore_param.sub_msg_cnt] = id;
                rwnx_hw->all_restore_param.sub_msg_cnt++;
                memcpy(&rwnx_hw->all_restore_param.vendor_swconfig_req[rwnx_hw->all_restore_param.vendor_swconfig_req_cnt], req, sizeof(struct mm_set_vendor_swconfig_req));
                rwnx_hw->all_restore_param.vendor_swconfig_req_cnt++;
            }
            break;
        }
        default:
        ...
        for (i = 0; i < rwnx_hw->all_restore_param.sub_msg_cnt; i++) {
            if (rwnx_hw->all_restore_param.sub_msg_id[i] == id) {
                found = true;
                break;
            }
        }
        ...
        switch (id) {
        case MM_SET_STACK_START_REQ :
            memcpy(&rwnx_hw->all_restore_param.stack_start_req, param, param_len);
            break;
        case MM_SET_TXPWR_IDX_LVL_REQ:
            memcpy(&rwnx_hw->all_restore_param.txpwr_lvl_req, param, param_len);
            break;
        case MM_SET_TXPWR_LVL_ADJ_REQ:
            memcpy(&rwnx_hw->all_restore_param.txpwr_lvl_adj_req, param, param_len);
            break;
        case MM_SET_TXPWR_OFST_REQ:
            memcpy(&rwnx_hw->all_restore_param.txpwr_ofst_req, param, param_len);
            break;
        case MM_SET_RF_CALIB_REQ:
            memcpy(&rwnx_hw->all_restore_param.rf_calib_req, param, param_len);
            break;
        case MM_RESET_REQ:
            break;
        case ME_CONFIG_REQ:
            memcpy(&rwnx_hw->all_restore_param.config_req, param, param_len);
            break;
        case ME_CHAN_CONFIG_REQ:
            memcpy(&rwnx_hw->all_restore_param.chan_config_req, param, param_len);
            break;
        case MM_SET_COEX_REQ:
            memcpy(&rwnx_hw->all_restore_param.coex_req, param, param_len);
            break;
        case MM_START_REQ:
            memcpy(&rwnx_hw->all_restore_param.start_req, param, param_len);
            break;
        case DBG_PWM_INIT_REQ:
            memcpy(&rwnx_hw->all_restore_param.pwm_init_req, param, param_len);
            break;
        default:
            break;
        }
...
#if 0
    printk("mm_all_restore_param:%d\n", sizeof(struct mm_all_restore_param));
    printk("offset: %d %d %d %d | %d %d %d %d | %d %d %d %d | %d %d %d %d\n",
                        offsetof(struct mm_all_restore_param, stack_start_req),
                        offsetof(struct mm_all_restore_param, txpwr_lvl_req),
                        offsetof(struct mm_all_restore_param, txpwr_lvl_adj_req),
                        offsetof(struct mm_all_restore_param, txpwr_ofst_req),
                        offsetof(struct mm_all_restore_param, rf_calib_req),
                        offsetof(struct mm_all_restore_param, config_req),
                        offsetof(struct mm_all_restore_param, chan_config_req),
                        offsetof(struct mm_all_restore_param, coex_req),
                        offsetof(struct mm_all_restore_param, start_req),
                        offsetof(struct mm_all_restore_param, vendor_swconfig_req),
                        offsetof(struct mm_all_restore_param, pwm_init_req),
                        offsetof(struct mm_all_restore_param, add_if_req),
                        offsetof(struct mm_all_restore_param, sub_msg_id),
                        offsetof(struct mm_all_restore_param, sub_msg_cnt),
                        offsetof(struct mm_all_restore_param, vendor_swconfig_req_cnt),
                        offsetof(struct mm_all_restore_param, add_vif_cnt));
#endif
...
int rwnx_send_wifi_disable_req(struct rwnx_hw *rwnx_hw, struct dbg_wifi_disable_cfm *cfm)
{
	void *req;
	int ret;
	//RWNX_DBG(RWNX_FN_ENTRY_STR);

	req = rwnx_msg_zalloc(MM_GET_WIFI_DISABLE_REQ, TASK_MM, DRV_TASK_ID, sizeof(u8));
	if (!req)
		return -ENOMEM;

	ret = rwnx_send_msg(rwnx_hw, req, 1, MM_GET_WIFI_DISABLE_CFM, cfm);

	return ret;
}

bool rwnx_rfkill_check(struct rwnx_hw *rwnx_hw, u8 *valid)
{
	bool ret = false;
	int err;
	struct dbg_wifi_disable_cfm cfm;

	//RWNX_DBG(RWNX_FN_ENTRY_STR);

	err = rwnx_send_wifi_disable_req(rwnx_hw, &cfm);
